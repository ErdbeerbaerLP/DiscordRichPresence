...
public class RequestMessage {
	...
	public Object onMessageReceived(RequestMessage a, Supplier<Context> b) {
		if (a.toSend.equals("DRPC-Message-Request") && !DRPC.isClient) {
			...
			DRPC.MSG.sendTo(new Message_Message(ServerConfig.SERVER_MESSAGE.get()),
					b.get().getSender().connection.netManager, NetworkDirection.PLAY_TO_CLIENT);
			DRPC.ICON.sendTo(new Message_Icon(ServerConfig.SERVER_ICON.get()),
					b.get().getSender().connection.netManager, NetworkDirection.PLAY_TO_CLIENT);
		}
		return null;
	}
