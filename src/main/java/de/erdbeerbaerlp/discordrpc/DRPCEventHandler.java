			...
			if (checkedUpdate == false) {
				...
				if (result == Status.OUTDATED) {
					event.player.sendMessage(new TextComponentString(
							"\u00A76[\u00A75DiscordRichPresence\u00A76]\u00A7c Update available!\n\u00A7cCurrent version: \u00A74"
									+ DRPC.VERSION + "\u00A7c, Newest: \u00A7a" + ForgeVersion.getTarget()
									+ "\n\u00A7cChangelog:\n\u00A76" + ForgeVersion.getSpec())
											.setStyle(new Style()
													.setClickEvent(new ClickEvent(Action.OPEN_URL,
															"https://minecraft.curseforge.com/projects/discordrpc"))
													.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
															new TextComponentString("Click to get newer Version")))));
					DRPCLog.Fatal(
							"UpdateCheck: Update Available. Download it here: https://minecraft.curseforge.com/projects/discordrichpresence/files");
					checkedUpdate = true;
				} else if (result == Status.AHEAD) {
					event.player.sendMessage(new TextComponentString(
							"\u00A76[\u00A75DiscordRichPresence\u00A76]\u00A77 It looks like you are using an Development version... \n\u00A77Your version: \u00A76"
									+ DRPC.VERSION)
											.setStyle(new Style()
													...
													.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
															new TextComponentString(
																	"Click to get current stable Version")))));
					DRPCLog.Fatal("UpdateCheck: Unreleased version... Assuming Development Version");
					checkedUpdate = true;
				} else if (result == Status.FAILED) {
					event.player.sendMessage(new TextComponentString(
							"\u00A76[\u00A75DiscordRichPresence\u00A76]\u00A7c FAILED TO CHECK FOR UPDATES"));
					DRPCLog.Fatal("UpdateCheck: Unable to check for updates");
					checkedUpdate = true;
				} else if (result == Status.BETA) {
					event.player.sendMessage(new TextComponentString(
							"\u00A76[\u00A75DiscordRichPresence\u00A76]\u00A7a You are using an Beta Version. This may contain bugs which are being fixed."));
					DRPCLog.Fatal("UpdateCheck: Beta");
					checkedUpdate = true;
				} else if (result == Status.BETA_OUTDATED) {
					event.player.sendMessage(new TextComponentString(
							"\u00A76[\u00A75DiscordRichPresence\u00A76]\u00A7c You are using an Outdated Beta Version. This may contain bugs which are being fixed or are already fixed\n\u00A76Changelog of newer Beta:"
									+ ForgeVersion.getSpec()));
			...
			if (DRPC.isEnabled) {
				try {
					...
					if (usingCustomMsg == false && serverCustomMessage.equals("") == false) {
						...
						Discord.setPresence(ClientConfig.NAME.get(), serverCustomMessage
								.replace("%players%", online + "").replace("%otherpl%", (online - 1) + ""), customIco);
						usingCustomMsg = true;
					}
//				System.out.println(Minecraft.getInstance().getCurrentServerData());
					if (Minecraft.getInstance().getCurrentServerData() == null) {
						if (tickAmount <= 0) {
							...
							Discord.setPresence(ClientConfig.NAME.get(),
									ClientConfig.WORLD_MESSAGE.get().replace("%world%", iServer.getFolderName())
						...
						if (tickAmount == 0) {
							...
							if (!serverCustomMessage.isEmpty()) {
								DRPCLog.Debug("CustomMSG Applied");
								Discord.setPresence(ClientConfig.NAME.get(), serverCustomMessage
										.replace("%players%", online + "").replace("%otherpl%", (online - 1) + ""),
										customIco);
								usingCustomMsg = true;
							} else {
								if (Minecraft.getInstance().getCurrentServerData().serverIP.toLowerCase()
										.contains("hypixel.net")) {
//								Log.Fatal("HYPIXEL!!!!!!!!!");
									String scoreboardTitle = null;
									try {
										scoreboardTitle = Minecraft.getInstance().world.getScoreboard()
												.getObjectiveInDisplaySlot(1).getDisplayName()
												.getUnformattedComponentText();
									} catch (NullPointerException e) {
										scoreboardTitle = "";
									}
									if (scoreboardTitle.equals("HYPIXEL") || scoreboardTitle.equals("PROTOTYPE")
											|| scoreboardTitle.equals("THE TNT GAMES")
											|| scoreboardTitle.equals("ARCADE GAMES")
											|| scoreboardTitle.equals("CLASSIC GAMES"))
										scoreboardTitle = "HUB";
									...
									Discord.setPresence(ClientConfig.NAME.get(),
											"Hypixel [" + scoreboardTitle + "] with " + (online - 1) + " other players",
											"49tz49873897485");
								} else if (Minecraft.getInstance().getCurrentServerData().serverIP.toLowerCase()
										.contains("mineplex.com")) {
									Discord.setPresence(ClientConfig.NAME.get(),
											"Playing on Mineplex with " + (online - 1) + " other players",
											"23498365347867869");
								} else if (Minecraft.getInstance().getCurrentServerData().serverIP.toLowerCase()
										.contains("wynncraft.com")) {
									Discord.setPresence(ClientConfig.NAME.get(),
											"Playing on Wynncraft, The Minecraft MMORPG", "4878hz4389634tz987");
								} else if (Minecraft.getInstance().getCurrentServerData().serverIP.toLowerCase()
										.contains("hivemc.com")) {
									try {
										JsonParser parse = new JsonParser();
										URL gameURL = new URL("https://api.hivemc.com/v1/player/"
												+ Minecraft.getInstance().player.getName() + "/status/raw?v=1");
										HttpURLConnection gameConn = (HttpURLConnection) gameURL.openConnection();
										gameConn.setRequestProperty("User-Agent",
												"ErdbeerbaerLP-DiscordRichPresence-Mod");
										InputStream isGame = gameConn.getInputStream();
										BufferedReader r = new BufferedReader(
												new InputStreamReader(isGame, Charset.forName("UTF-8")));
										...
										if (gameO.equals("BEDT"))
											gameO = "BedWars";
										Discord.setPresence("TheHive", "Playing " + gameO + " on hivemc.com",
												"38462896734683686");
									...
									Discord.setPresence(ClientConfig.NAME.get(),
											ClientConfig.SERVER_MESSAGE.get().replace("%ip%",
													Minecraft.getInstance().getCurrentServerData().serverIP) + "("
													+ online + "/" + maxPlayers + " players)",
											"cube");
								}
							}
							tickAmount = 2000;
	...
	public static String removeFormatting(String formatted) {
		return formatted.replaceAll("\u00A70", "").replaceAll("\u00A71", "").replaceAll("\u00A72", "")
				.replaceAll("\u00A73", "").replaceAll("\u00A74", "").replaceAll("\u00A75", "").replaceAll("\u00A76", "")
				.replaceAll("\u00A77", "").replaceAll("\u00A78", "").replaceAll("\u00A79", "").replaceAll("\u00A7a", "")
				.replaceAll("\u00A7b", "").replaceAll("\u00A7c", "").replaceAll("\u00A7d", "").replaceAll("\u00A7e", "")
				.replaceAll("\u00A7f", "").replaceAll("\u00A7l", "").replaceAll("\u00A7k", "").replaceAll("\u00A7m", "")
